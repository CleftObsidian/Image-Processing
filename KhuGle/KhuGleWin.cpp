...
void CKhuGleWin::DrawSceneTextPos(char *Text, CKgPoint ptPos)
{
	...
	Rt.left = 0;
	Rt.top = 0;
	DrawText(hCompDC, (LPCSTR)Text, strlen(Text), &Rt, DT_CALCRECT | DT_LEFT);
	...
	DrawText(hCompDC, (LPCSTR)Text, strlen(Text), &Rt, DT_LEFT);
	...
	int       nCmdShow)
{
	...
	windowClass.lpszClassName = (LPCSTR)"WinClass";
	...
	CKhuGleWin::m_pWinApplication->m_hWnd = CreateWindowEx(NULL, windowClass.lpszClassName,
		(LPCSTR)"Ai and Data via Game",
		dwStyle |
		WS_CLIPCHILDREN |
		WS_CLIPSIBLINGS,
		0, 0,                  
